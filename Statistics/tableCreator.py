...
def makeTable(sourceFile,outputPath,outputName):
    ...
    for line in lines:
        ...
        if modality=="Sequential":
            seqTime=float(elem[3])
            nuovo.append("1")
        else:
            parallelTime=float(elem[3])
    ...
    density = int(float(name[2]) * 100)
    title=name[0]+" nodes & " + str(density) + "% density"
...
for path, currentDirectory, files in os.walk("TimeResults"):
    for file in files:
        resultPath=path.replace("TimeResults","Tables")
